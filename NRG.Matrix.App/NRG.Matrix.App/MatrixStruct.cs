...
public class MatrixStruct(int delay = 80, TimeSpan? time = null, int maxObjects = 9999, Func<int, float>? objectAddRate = null)
{
	private readonly bool _isEndlessMode = time is null;

	private List<DisplayObjectStruct> list = [];
	...
	private void AddObjects(int width)
	{
        ...
        {
			var obj = new DisplayObjectStruct(width);
			list.Add(obj);
	...
	private void PrintTrace(IGrouping<ConsoleColor, DisplayObjectStruct>? traces)
	{
		if (traces is null)
		{
			return;
	...
	private static string GetLineText(List<DisplayObjectStruct> obj, int first, int last)
	{
	...
	private void PrintOthers(IEnumerable<IGrouping<ConsoleColor, DisplayObjectStruct>> others)
	{
