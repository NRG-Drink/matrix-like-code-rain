...
public class MatrixClass(int delay = 80, TimeSpan? time = null, int maxObjects = 9999, Func<int, float>? objectAddRate = null)
{
	private readonly bool _isEndlessMode = time is null;

	private List<DisplayObjectClass> list = [];
	...
	private void AddObjects(int width)
	{
        ...
        {
			var obj = new DisplayObjectClass(width);
			list.Add(obj);
	...
	private void PrintTrace(IGrouping<ConsoleColor, DisplayObjectClass>? traces)
	{
		if (traces is null)
		{
			return;
	...
	private static string GetLineText(List<DisplayObjectClass> obj, int first, int last)
	{
	...
	private void PrintOthers(IEnumerable<IGrouping<ConsoleColor, DisplayObjectClass>> others)
	{
